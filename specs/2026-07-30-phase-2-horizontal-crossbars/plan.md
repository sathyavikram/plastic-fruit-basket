...
3. Construct the outer main solid cylinder along the X-axis:
   - Outer diameter: `CROSSBAR_DIAMETER = 18.0 * SCALE` (radius $9.0\text{ mm}$)
...
## Task Group 2: Internal M12 Female Thread Sockets (`freecad-threading`)
1. Generate internal female thread cutter at origin using `freecad-threading` pattern:
   - Nominal major radius: `t_radius = SCREW_THREAD_DIAMETER / 2.0 = 6.0 * SCALE`
   - Pitch: `THREAD_PITCH = 2.5 * SCALE`
...
2. Cut M12 female thread socket into the left end ($X = 0$) to 25 mm depth.
3. Cut M12 female thread socket into the right end ($X = 170.0\text{ mm}$) to 25 mm depth.
