...
EXPORT_STEP = os.path.join(EXPORT_BASE, "part_02_stand_middle.step")
EXPORT_STL  = os.path.join(EXPORT_BASE, "part_02_stand_middle.stl")


def construct_middle_stand():
    """
    Constructs part_02_stand_middle (Z=125mm to Z=250mm).
    Middle stand tier with bottom female mortise alignment socket, middle curved riser,
    extended cradle arm with upward-curving tip, top-surface 60-degree dovetail slots,
    and top male tenon alignment peg.
    """
    ...
    # 3. Bottom Female Mortise Socket at Z=125mm
    ...
    # 5. Top Male Alignment Tenon Peg at Z=250mm
...
def main():
    doc = App.newDocument("MiddleStand")
    shape = construct_middle_stand()
    feature = doc.addObject("Part::Feature", "MiddleStand")
    feature.Shape = shape
    doc.recompute()
